 ...
 */
type recentFoodType = [string, number, boolean]
const recentFood: recentFoodType = ['치킨', 19000, true];

/**
 * Q2. 이렇게 생긴 자료는 타입지정 어떻게 해야할까요?
 */
type arrType = [string, number, ...boolean[]]
const arr: arrType = ['동서녹차', 4000, true, false, true, true, false, true];
 ...
 */
type q3Type = [string, boolean, ...(number | string)[]];

function q3(...rest: q3Type) {
}
...
function q4(...rest: (string | number)[]) {
    let resultArr: [string[], number[]] = [[], []];
    rest.forEach(item => {
        if (typeof item === "string") resultArr[0].push(item);
        else resultArr[1].push(item);
    })
    return resultArr;
}
